...
// Import game assets
const map5Bg = require('../assets/game pngs/map5.5.png');
...
export default function Map5Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions5');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 5 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions5', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 5 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map5Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 5 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 5 - Stage Position Debug</Text>
