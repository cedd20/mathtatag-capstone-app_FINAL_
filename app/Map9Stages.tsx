...
// Import game assets
const map9Bg = require('../assets/game pngs/map9.9.png');
...
export default function Map9Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions9');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 9 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions9', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 9 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map9Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 9 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 9 - Stage Position Debug</Text>
