...
// Import game assets
const map12Bg = require('../assets/game pngs/map12.12.png');
...
export default function Map12Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions12');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 12 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions12', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 12 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map12Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 12 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 12 - Stage Position Debug</Text>
