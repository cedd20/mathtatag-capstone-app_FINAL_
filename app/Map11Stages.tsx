...
// Import game assets
const map11Bg = require('../assets/game pngs/map11.11.png');
...
export default function Map11Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions11');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 11 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions11', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 11 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map11Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 11 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 11 - Stage Position Debug</Text>
