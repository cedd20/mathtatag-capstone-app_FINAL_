...
// Import game assets
const map8Bg = require('../assets/game pngs/map8.8.png');
...
export default function Map8Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions8');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 8 - Current stage positions:');
  ...
  return (
    <ImageBackground source={map8Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 8 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 8 - Stage Position Debug</Text>
