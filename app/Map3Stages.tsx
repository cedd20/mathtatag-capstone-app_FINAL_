...
// Import game assets
const map3Bg = require('../assets/game pngs/map3.3.png');
...
export default function Map3Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions3');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 3 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions3', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 3 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map3Bg} style={styles.bg} resizeMode="stretch">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 3 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 3 - Stage Position Debug</Text>
