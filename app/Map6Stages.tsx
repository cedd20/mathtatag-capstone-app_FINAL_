...
// Import game assets
const map6Bg = require('../assets/game pngs/map6.6.png');
...
export default function Map6Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions6');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 6 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions6', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 6 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map6Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 6 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 6 - Stage Position Debug</Text>
