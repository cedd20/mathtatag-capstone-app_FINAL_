...
// Import game assets
const map7Bg = require('../assets/game pngs/map7.7.png');
...
export default function Map7Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions7');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 7 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions7', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 7 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map7Bg} style={styles.bg} resizeMode="cover">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 7 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 7 - Stage Position Debug</Text>
