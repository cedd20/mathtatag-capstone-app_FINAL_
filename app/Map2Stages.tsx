...
// Import game assets
const map2Bg = require('../assets/game pngs/map2.2.png');
...
export default function Map2Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions2');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 2 - Current stage positions:');
  ...
  return (
    <ImageBackground source={map2Bg} style={styles.bg} resizeMode="stretch">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 2 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 2 - Stage Position Debug</Text>
