...
// Import game assets
const map4Bg = require('../assets/game pngs/map4.4.png');
...
export default function Map4Stages() {
  ...
  useEffect(() => {
    const loadSavedPositions = async () => {
      try {
        const saved = await AsyncStorage.getItem('stagePositions4');
        if (saved) {
  ...
  const logStagePositions = () => {
    console.log('Week 4 - Current stage positions:');
  ...
  const saveStagePositions = async () => {
    try {
      await AsyncStorage.setItem('stagePositions4', JSON.stringify(stagePositions));
      setStagePositions([...stagePositions]);
      console.log('Week 4 positions saved successfully!');
    } catch (e) {
  ...
  return (
    <ImageBackground source={map4Bg} style={styles.bg} resizeMode="stretch">
      ...
      {showCongrats && (
        <View style={congratsStyles.card}>
          ...
          <Text style={congratsStyles.text}>You finished all Week 4 games!</Text>
      ...
      {debugMode && (
        <View style={debugStyles.debugPanel}>
          <Text style={debugStyles.debugTitle}>Week 4 - Stage Position Debug</Text>
